...
```{r, echo = FALSE}
flair::flair_lines(flair::decorate_chunk("v2-chunk", eval = FALSE), 7)
```
...
short <- MCMCEnsemble(
  p.log, 
  inits = runif(n_walkers, min = c(-5, -15), max = c(5, 15)),
  max.iter = 50, n.walkers = n_walkers,
  method = "stretch", coda = TRUE
)
```
...
```{r, echo = FALSE}
flair::flair_lines(flair::decorate_chunk("v3-chunk", eval = FALSE), 7)
```
